...
public class ScrabbleGame {
    ...
    public static void startGame(){
        ...
        // runs binary search to see if userWord is valid    
        Word wordToCheck = new Word(userWord);

        // Checks if the inputted word contains the random characters
        int count = 0;
        for (char c : randomChars) {
            if (wordToCheck.indexOf(c) != -1) {
                count++;
            }
        }

        if (binarySearch(words, wordToCheck) >= 0 && count == 4) {
